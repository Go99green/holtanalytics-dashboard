...
const AUTHOR = {
  ...
  title:
    "MBA Candidate · Commercial Strategy, Operations, and Business Intelligence",
...
function getMetricByKey(
  rows: ReturnType<typeof loadDashboardData>["masterModel"],
  key: string
) {
...
function buildLinePath(
  ...
  return values
    .map((value, index) => {
      const x =
        padding + (index / Math.max(values.length - 1, 1)) * usableWidth;
  ...
  return (
    <button
      ...
      className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${
        active
          ? "border-[#F47A38] bg-[#F47A38]/20 text-orange-100 shadow-[0_0_0_1px_rgba(244,122,56,.15)]"
...
function TooltipLabel({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        title={tooltip}
        className="inline-flex h-5 w-5 cursor-help items-center justify-center rounded-full border border-slate-600 text-[10px] font-bold text-slate-300"
      >
        i
      </span>
    </div>
...
export default function Page() {
  ...
  const initialIssueId = data.issueBacklog[0]?.id ?? 1;

  const [scenario, setScenario] = useState<ScenarioName>("Tactical");
  const [activeSection, setActiveSection] =
    useState<SectionKey>("leadership");
  const [compareBaseline, setCompareBaseline] = useState(true);
  const [openMethodology, setOpenMethodology] = useState(false);
  const [activeIssue, setActiveIssue] = useState<number>(initialIssueId);
  const [hoverIndex, setHoverIndex] = useState<number | null>(null);

  const attendanceSeries = getMetricByKey(
    data.masterModel,
    "1_average_attendance"
  )?.valuesByYear;
  const watpSeries = getMetricByKey(
    data.masterModel,
    "20_weighted_average_ticket_price_modeled"
  )?.valuesByYear;
  const revenueSeries = getMetricByKey(
    data.masterModel,
    "46_gross_annual_revenue"
  )?.valuesByYear;
  ...
  const projectionIndex = new Map(
    data.fy27ScenarioProjections.map((item) => [
      item.metricKey,
      item.valuesByScenario,
    ])
  );

  const selectedProjection = {
    attendance: Number(
      projectionIndex.get("projected_attendance")?.[scenario] ?? 0
    ),
    watp: Number(projectionIndex.get("projected_watp")?.[scenario] ?? 0),
    revenue: Number(
      projectionIndex.get("projected_total_revenue")?.[scenario] ?? 0
    ),
    ebitda: Number(projectionIndex.get("projected_ebitda")?.[scenario] ?? 0),
    valuation: Number(
      projectionIndex.get("projected_valuation")?.[scenario] ?? 0
    ),
  };

  const baselineProjection = {
    attendance: Number(
      projectionIndex.get("projected_attendance")?.Baseline ?? 0
    ),
    watp: Number(projectionIndex.get("projected_watp")?.Baseline ?? 0),
    revenue: Number(
      projectionIndex.get("projected_total_revenue")?.Baseline ?? 0
    ),
    ebitda: Number(projectionIndex.get("projected_ebitda")?.Baseline ?? 0),
    valuation: Number(
      projectionIndex.get("projected_valuation")?.Baseline ?? 0
    ),
  };

  const trendWithScenario = [
    ...historicalTrend,
    { year: "2026-27", ...selectedProjection },
  ];

  const revenueDeltaVsBaseline =
    baselineProjection.revenue > 0
      ? (selectedProjection.revenue - baselineProjection.revenue) /
        baselineProjection.revenue
      : 0;

  const ebitdaMargin =
    selectedProjection.revenue > 0
      ? selectedProjection.ebitda / selectedProjection.revenue
      : 0;
  ...
  const cards: MetricCard[] = [
    {
      ...
      delta: compareBaseline
        ? `${PERCENT.format(revenueDeltaVsBaseline)} vs Baseline`
        : undefined,
    ...
    },
    {
      ...
      detail: "Illustrative revenue-multiple benchmark",
      tooltip:
  ...
  const activeIssueDetails = data.issueBacklog.find(
    (issue) => issue.id === activeIssue
  );
  ...
  const revenuePath = buildLinePath(revenuePoints, width, height, padding);
  const attendancePath = buildLinePath(attendancePoints, width, height, padding);

  const selectedScenarioHeading =
    scenario === "Baseline" ? "Selected (Baseline)" : scenario;
      ...
      <div className="mx-auto w-full max-w-7xl px-6 py-8 lg:px-10">
            ...
            <div className="max-w-3xl space-y-4">
              <div className="flex items-center gap-3">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-2">
                  <Image
                    src="/San_Diego_Gulls_logo.svg.png"
                    alt="San Diego Gulls logo"
                    width={56}
                    height={56}
              ...
              <h1 className="text-3xl font-bold leading-tight text-white lg:text-5xl">
                Closing the Revenue Gap: Data-Driven Strategies to Scale Gulls
                Commercial Performance Through FY27
              </h1>

              <p className="max-w-2xl text-sm text-slate-300 lg:text-base">
                Decision dashboard for ticketing, partnerships, and operations
                leadership to improve conversion, per-cap economics, and margin
                preservation through FY27 scenario planning.
              </p>

              <div className="space-y-2">
                <p className="text-sm font-medium text-slate-100">
                  Prepared by {AUTHOR.name} · {AUTHOR.company}
                </p>
                <p className="text-xs text-slate-400">
                  Coverage: FY2021-22 through FY2026-27 scenarios · Last
                  updated: April 2026
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-slate-300">
                Built from historical anchors, modeled estimates, and FY27
                scenario projections. Intended as a decision-support dashboard,
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {cards.map((card) => (
            <article
              key={card.label}
              className="rounded-2xl border border-slate-700/60 bg-[#111827]/70 p-5"
            >
              <TooltipLabel label={card.label} tooltip={card.tooltip} />
              <p className="mt-3 text-3xl font-bold text-white">{card.value}</p>
              {card.delta ? (
                <p className="mt-2 text-sm font-semibold text-[#F47A38]">
                  {card.delta}
                </p>
              ) : null}
          ...
          <section className="mt-6 rounded-3xl border border-slate-700/60 bg-[#111827]/70 p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-white">
                Historical vs FY27 Scenario Trend
              </h2>
            ...
            >
              ...
              {trendWithScenario.map((point, index) => {
                const x =
                  padding +
                  (index / Math.max(trendWithScenario.length - 1, 1)) *
                    (width - padding * 2);
                ...
                const revRatio =
                  revMax === revMin
                    ? 0.5
                    : (point.revenue - revMin) / (revMax - revMin);
                const y =
                  height - padding - revRatio * (height - padding * 2);
            ...
            {hoverIndex !== null ? (
              <div className="mt-4 rounded-xl border border-slate-600 bg-slate-950/70 p-4 text-sm text-slate-200">
                <p className="font-semibold text-white">
                  {trendWithScenario[hoverIndex].year}
                </p>
                <p>
                  Revenue:{" "}
                  {formatMillions(trendWithScenario[hoverIndex].revenue)}
                </p>
                <p>
                  Attendance:{" "}
                  {formatNumber(trendWithScenario[hoverIndex].attendance)}
                </p>
                <p>
                  WATP: {CURRENCY.format(trendWithScenario[hoverIndex].watp)}
                </p>
              </div>
            ) : null}
          </section>
        ...
        {(activeSection === "leadership" ||
          ...
          <section className="mt-6 rounded-3xl border border-slate-700/60 bg-[#111827]/70 p-6">
            ...
            <p className="mt-2 text-sm text-slate-300">
              Highest-value commercial and operational moves to convert demand
              into predictable revenue and operating leverage.
            </p>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {recommendations.map((item) => (
                ...
                >
                  <p className="text-xs uppercase tracking-[0.18em] text-[#F47A38]">
                    {item.pillar}
                  </p>
              ...
              {activeIssueDetails ? (
                <div className="mt-4 space-y-3 text-sm text-slate-200">
                  ...
                  <p>
                    ...
                    Midway Rising and the Pechanga legacy window require
                    defending premium yield and operating leverage before a
                    future facility transition.
                  </p>
                </div>
              ) : null}
            </div>
              ...
              <table className="min-w-full text-left text-sm">
                <thead className="text-slate-300">
                  <tr>
                    ...
                    <th className="pb-3 pr-6">{selectedScenarioHeading}</th>
                ...
                <tbody className="text-slate-100">
                  {[
                    [
                      "Revenue",
                      baselineProjection.revenue,
                      selectedProjection.revenue,
                      formatMillions,
                    ],
                    [
                      "EBITDA",
                      baselineProjection.ebitda,
                      selectedProjection.ebitda,
                      formatMillions,
                    ],
                    [
                    ...
                    [
                      "Attendance",
                      baselineProjection.attendance,
                      selectedProjection.attendance,
                      formatNumber,
                    ],
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 pr-6">
                          {(formatter as (n: number) => string)(Number(base))}
                        </td>
                        <td className="py-3 pr-6">
                          {(formatter as (n: number) => string)(
                            Number(selected)
                          )}
                        </td>
                        <td
        ...
        {(activeSection === "methodology" ||
          ...
          <section className="mt-6 rounded-3xl border border-slate-700/60 bg-[#111827]/70 p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h2 className="text-xl font-semibold text-white">
                  Data Sources & Trust Framework
                </h2>
                <p className="mt-2 text-sm text-slate-300">
                  Public anchors, modeled estimates, and scenario projections
                  are separated to preserve decision quality and transparency.
                </p>
              </div>
              <button
            ...
            {openMethodology ? (
              <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                <div className="rounded-xl border border-slate-700 bg-slate-950/70 p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-300">
                    Public AHL attendance records, team schedule/ticket pages,
                    and official market context.
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-slate-700 bg-slate-950/70 p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-300">
                    Structured estimates for monetization drivers, retention,
                    yield, and per-cap dynamics.
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-slate-700 bg-slate-950/70 p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-300">
                    Baseline, Tactical, and Aggressive outputs from workbook
                    scenario sheets.
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-slate-700 bg-slate-950/70 p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-300">
                    Control rates and assumption rows are loaded from normalized
                    workbook artifacts.
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-slate-700 bg-slate-950/70 p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-300">
                    Formula-driven workbook cells rely on cached values where
                    applicable; interpretation is decision-support, not audited
                    finance.
                  </p>
                </div>
              </div>
        ...
        {(activeSection === "author" || activeSection === "leadership") && (
          <footer className="mt-8 rounded-3xl border border-slate-700/60 bg-slate-950/70 p-6">
            <div className="grid gap-5 lg:grid-cols-[220px_1fr_0.9fr]">
              ...
              <div>
                ...
                <p className="mt-3 text-sm leading-7 text-slate-400">
                  This executive interface is designed as a decision-support
                  system focused on revenue, operations, and scenario planning
                  for the San Diego Gulls.
                </p>
                <p className="mt-3 text-sm leading-7 text-slate-300">
                  Why this profile fits the room: MBA-candidate strategic
                  revenue lens, Army veteran military audience credibility,
                  CrossFit performance-culture insight, and creative background
                  bridging analytics with content and brand execution.
                </p>
              </div>
              ...
              <div className="space-y-2 text-sm text-slate-300">
                ...
                <p className="pt-2 text-xs leading-6 text-slate-400">
                  Commercial concept: Tactical Flex Pass / Ready Reserve Flex
                  Pack (10-game digital wallet design for active-duty, reserve,
                  and transient military households).
                </p>
                <p className="text-xs leading-6 text-slate-400">
                  Ops integration: connect ticketing, parking, POS/per-cap, and
                  Legends hospitality data to improve sponsor yield and margin
                  preservation.
                </p>
              </div>
            </div>
